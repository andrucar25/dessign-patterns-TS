...
class LocationIcon implements Location {
  ...
  display(coordinates: { x: number; y: number; }): void {
    console.log(
      `Coords: ${this.type} en ${coordinates.x}, ${coordinates.y} con icono ${this.iconImage}`
    )
  }
}
...
class LocationFactory {
  ...
  getLocationIcon(type: string): LocationIcon {
    if(!this.icons[type]){
      console.log(`Creando nueva instancia de icono de ${type}`)
...
function main() {
  ...
  const locations = [
    ...
    new MapLocation(45, 75, factory.getLocationIcon('escuela')),
