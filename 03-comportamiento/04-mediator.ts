...
//Chatroom es el mediador entre los usuarios
class ChatRoom {
  private users: User[] = []
  public title: string;
...
class User {
  ...
  sendMessage(message: string): void {
    console.log(`${this.username} envia: ${message}`);
...
function main() {
  ...
  const user1 = new User('Eduardo', chatRoom);
  const user2 = new User('Gaston', chatRoom);
  const user3 = new User('Mariangel', chatRoom);

  user1.sendMessage('Hola a todos');
  user2.sendMessage('Hola Eduardo');
  user3.sendMessage('Hola Eduardo y gaston');
